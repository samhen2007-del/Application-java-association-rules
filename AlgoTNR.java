...
import AssociationRules_Algos.ArraysAlgos;
import AssociationRules_Algos.RedBlackTree;
import AssociationRules_Algos.MemoryLogger;
...
public class AlgoTNR {
	...
	// for statistics
	public long timeStart = 0;  // start time of last execution
	public long timeEnd = 0;   // end time of last execution
	 public long TimeTotale;

	
	// the maximum number of candidates at the same time during the last execution
	public int maxCandidateCount = 0;
	public int notAdded = 0;  // rules eliminated by strategy 1
	public int totalremovedCount = 0; // rules eliminated by strategy 2
	
	public long totalCandidatesConsideredFromR = 0;  // the total number of candidates processed
	public long totalRules11considered = 0;  // the total number of rules with only two items considered
	
	// Parameters
	public double minConfidence;  // minimum confidence threshold
	public int initialK = 0;     // the value of k set by the user
	public Database database;    // the transaction database
	public int delta = 0;        // the delta parameter
	
	// Internal variables
	public RedBlackTree<RuleG> kRules;  // the top k rules found until now 
	public RedBlackTree<RuleG> candidates;  // the candidates for expansion
	public int k=0;              // will contain k + delta
	public int minsuppRelative;   // minimum support threshold that will be raised dynamically
	...
	public RedBlackTree<RuleG> runAlgorithm(int k, double minConfidence, Database database, int delta) {
		...
		notAdded = 0;
		x = new MemoryLogger();
		x.reset(); // reset utility to check memory usage
		maxCandidateCount = 0;
...
//		}
	...
	public void printStats() {
		System.out.println("==  Algorithme TOP-K RULES-NR ==");
		System.out.println("Support Minimum : " + minsuppRelative);
		System.out.println("Nombre de Regles : " + kRules.size());
		TimeTotale = timeEnd - timeStart ;
		System.out.println("Temps Totale : " + (TimeTotale ) + " s");
		System.out.println("Memoire : " + x.getMaxMemory() + " mb");
//		System.out.println("Candidates count : " + candidates.size());
		System.out.println("Regles supprimee par strategie 1: " + notAdded);
		System.out.println("Regles supprimee par strategie 2: " + totalremovedCount);	
		System.out.println("--------------------------------");
		System.out.println("===============================");
	}
}
